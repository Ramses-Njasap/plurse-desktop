...
// ── SubCategory Detail Modal (centered on offcanvas) ─────────────────────────
...
const SubCategoryModal = ({ sub, onClose, onUpdated, offcanvasRef }: SubModalProps) => {
  ...
  const set = (patch: Partial<SubEditForm>) => {
    setForm(f => ({ ...f, ...patch }))
    setDirty(true)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.profile_pic) {
        payload.update_image = true
        payload.image_data = form.profile_pic
      }
  ...
  // position modal centered on the offcanvas panel
  const getStyle = (): React.CSSProperties => {
    if (!offcanvasRef.current) return { position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }
    ...
    <>
      ...
      >
          ...
          <div className="flex items-center gap-2">
            {!editing && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-900 hover:bg-gray-800 text-white text-xs font-semibold rounded-lg transition-all shadow-sm"
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                </svg>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className={`px-5 pt-5 pb-4 ${editing ? 'bg-blue-50/40' : 'bg-gradient-to-b from-gray-50/80 to-white'} transition-colors`}>
            <div className="flex items-start gap-4">
              ...
              <div className="relative group flex-shrink-0">
                ...
                {category.is_deleted && <div className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 rounded-full border-2 border-white flex items-center justify-center"><svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg></div>}
                <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleFileChange} />
              </div>

              {/* name / description fields or read-only */}
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setF({ category_name: e.target.value })}
          ...
          {/* Save / Discard bar (only when editing + dirty) */}
          {editing && dirty && (
            <div className="px-5 py-3 bg-blue-50 border-y border-blue-100 flex items-center gap-3">
              ...
              <button onClick={handleDiscard} className="px-3 py-1.5 bg-white hover:bg-gray-50 text-gray-700 text-xs font-semibold border border-gray-300 rounded-lg transition-all shadow-sm">
                Discard
              </button>
          ...
          <div className="px-5 py-4 border-b border-gray-100 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <div className="font-medium text-gray-900">{category.parent_category_id ? `#${String(category.parent_category_id).padStart(4,'0')}` : <span className="text-gray-400">Root</span>}</div>
              </div>
              <div>
                  ...
                  {showSortMenu && (
                    <div className="absolute right-0 top-full mt-1.5 w-44 bg-white rounded-xl shadow-lg border border-gray-100 py-1.5 z-20">
                      {(Object.keys(sortLabels) as SortKey[]).map(key => (
                        <button
                          key={key}
                          onClick={() => { setSubSort(key); setShowSortMenu(false) }}
      ...
      {/* Sub-category detail modal — centered on offcanvas */}
      {selectedSub && (
        <SubCategoryModal
          sub={selectedSub}
