...
// API Response Type for Stock Purchase (matches what getAllStockPurchases returns)
...
// Our component's StockPurchase type (normalized)
type StockPurchase = {
  id: number
  sku_id: number
  quantity: number
...
const sortLabels: Record<SortKey, string> = {
  purchased_on_desc: 'Newest first', 
  purchased_on_asc: 'Oldest first',
  expiry_date_asc: 'Expiry (earliest)', 
  expiry_date_desc: 'Expiry (latest)',
  price_per_unit_asc: 'Price ↑', 
  price_per_unit_desc: 'Price ↓',
  profit_margin_asc: 'Margin ↑', 
  profit_margin_desc: 'Margin ↓',
}
...
const fmt$ = (v: number | null | undefined) => v == null ? '—' : new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2 }).format(v)
...
// Normalize API response to our StockPurchase type
const normalizePurchase = (apiPurchase: ApiStockPurchase): StockPurchase => {
  return {
    ...
    sku: apiPurchase.sku ? {
      id: apiPurchase.sku.id,
      name: apiPurchase.sku.sku_name,
      code: apiPurchase.sku.code,
      product: apiPurchase.sku.product_name ? {
        id: apiPurchase.sku.product_id,
        name: apiPurchase.sku.product_name
      } : null
    } : null,
    supplier: apiPurchase.supplier ? {
      id: apiPurchase.supplier.id,
      name: apiPurchase.supplier.supplier_name,
      contact: apiPurchase.supplier.contact_person
    } : null,
...
const SkuAvatar = ({ sku, size = 'md' }: { sku?: { name: string; images?: SkuImage[] } | null; size?: 'sm' | 'md' }) => {
  ...
  useEffect(() => {
    ...
    try { 
      const url = window.api.files.readFileAsDataURL((img as any).path); 
      setImgSrc(url) 
    } catch {} finally { 
      setLoaded(true) 
    }
...
const SkuDropdown = ({ value, onChange, error, required }: { value: number | null; onChange: (id: number | null, sku?: any) => void; error?: string; required?: boolean }) => {
        ...
        {loading ? <span className="text-gray-400">Loading SKUs…</span>
          : selected ? (
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0"><div className="text-sm font-medium text-gray-900 truncate">{selected.sku_name}</div><div className="text-xs text-gray-400 font-mono truncate">{selected.code}</div></div>
            </div>
      ...
      {open && !loading && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1.5 bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden">
          ...
          <div className="max-h-60 overflow-y-auto py-1">
            {value !== null && <button onClick={() => { onChange(null); setOpen(false); setSearch('') }} className="w-full px-3 py-2 text-left text-sm text-gray-400 hover:bg-gray-50 flex items-center gap-2 italic border-b border-gray-100"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>Clear</button>}
            {filtered.length === 0 ? (
              <div className="px-3 py-4 text-center"><p className="text-sm text-gray-400 mb-1">No SKUs found{search ? ` for "${search}"` : ''}</p><p className="text-xs text-blue-600 font-medium">Create a SKU from the Products page first</p></div>
            ) : filtered.map(sku => (
...
const SupplierSection = ({ form, set, errors }: { form: CreatePurchaseForm; set: (p: Partial<CreatePurchaseForm>) => void; errors: Record<string, string> }) => {
          ...
          {open && !loading && (
            <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden">
              ...
              <div className="max-h-48 overflow-y-auto py-1">
                ...
                {filtered.length === 0 ? <div className="px-3 py-3 text-center text-sm text-gray-400">No suppliers found</div> : filtered.map(s => (
...
const PurchaseModal = ({ open, onClose, onSuccess, editPurchase }: { open: boolean; onClose: () => void; onSuccess: (msg: string) => void; editPurchase?: StockPurchase | null }) => {
  ...
  return (
    <div className="fixed top-0 left-64 right-0 bottom-0 z-50 flex items-center justify-center p-4" style={{ animation: 'fadeIn 0.15s ease-out' }}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] flex flex-col overflow-hidden" style={{ animation: 'scaleIn 0.15s ease-out' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-bold text-gray-900">{isEdit ? 'Edit Stock Purchase' : 'New Stock Purchase'}</h2>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
          ...
          {!isEdit ? (
            <SkuDropdown value={form.sku_id} onChange={(id) => { set({ sku_id: id }); if (errors.sku_id) setErrors(e => { const n = {...e}; delete n.sku_id; return n }) }} error={errors.sku_id} required />
          ) : editPurchase?.sku && (
            <div className="flex items-center gap-2.5 px-3 py-2.5 bg-violet-50 rounded-lg border border-violet-200">
              <svg className="w-4 h-4 text-violet-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
              <div className="min-w-0"><p className="text-xs font-semibold text-violet-800 truncate">{editPurchase.sku.name}</p><p className="text-xs text-violet-600 font-mono">{editPurchase.sku.code}</p></div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min="1" value={form.quantity} onChange={e => { set({ quantity: e.target.value }); if (errors.quantity) setErrors(er => { const n = {...er}; delete n.quantity; return n }) }} placeholder="e.g. 100"
            ...
            <div>
              ...
              <input type="number" min="0" step="0.01" value={form.price_per_unit} onChange={e => { set({ price_per_unit: e.target.value }); if (errors.price_per_unit) setErrors(er => { const n = {...er}; delete n.price_per_unit; return n }) }} placeholder="0.00"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min="0" step="0.01" value={form.total_price_bought} onChange={e => { set({ total_price_bought: e.target.value }); if (errors.total_price_bought) setErrors(er => { const n = {...er}; delete n.total_price_bought; return n }) }} placeholder="Auto-calculated"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" min="0" step="0.01" value={form.min_price} onChange={e => { set({ min_price: e.target.value }); if (errors.min_price) setErrors(er => { const n = {...er}; delete n.min_price; return n }) }} placeholder="0.00"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-xs font-semibold text-gray-600 mb-1">Purchased On</label><input type="date" value={form.purchased_on} onChange={e => set({ purchased_on: e.target.value })} className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" /></div>
            <div><label className="block text-xs font-semibold text-gray-600 mb-1">Arrived On</label><input type="date" value={form.arrived_on} onChange={e => set({ arrived_on: e.target.value })} className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" /></div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-xs font-semibold text-gray-600 mb-1">Expiry Date</label><input type="date" value={form.expiry_date} onChange={e => set({ expiry_date: e.target.value })} className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" /></div>
            <div><label className="block text-xs font-semibold text-gray-600 mb-1">Manufacture Date</label><input type="date" value={form.manufacture_date} onChange={e => set({ manufacture_date: e.target.value })} className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white" /></div>
          </div>
        ...
        <div className="flex items-center justify-end gap-3 px-5 py-4 border-t border-gray-100 bg-gray-50/50 flex-shrink-0">
          ...
          <button onClick={handleSubmit} disabled={submitting}
            ...
            {submitting ? <><svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Saving…</>
            : <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7"/></svg>{isEdit ? 'Save Changes' : 'Create Purchase'}</>}
          </button>
        </div>
      </div>
...
const DeleteModal = ({ purchase, onConfirm, onCancel }: { purchase: StockPurchase; onConfirm: () => void; onCancel: () => void }) => (
  <div className="fixed top-0 left-64 right-0 bottom-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm" style={{ animation: 'fadeIn 0.15s ease-out' }} onClick={onCancel}>
    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden" style={{ animation: 'scaleIn 0.15s ease-out' }} onClick={e => e.stopPropagation()}>
      <div className="px-6 pt-6 pb-4 bg-gradient-to-br from-red-50 to-red-100/40">
        <div className="flex items-start gap-3">
          ...
          <div><h3 className="text-base font-bold text-gray-900">Delete Stock Purchase?</h3><p className="text-sm text-gray-500 mt-1">Purchase #{purchase.id} · {fmt$(purchase.total_price_bought)}</p></div>
        </div>
      </div>
      <div className="px-6 py-5">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 px-4 py-2.5 bg-white hover:bg-gray-50 text-gray-700 text-sm font-semibold border border-gray-300 rounded-lg transition-all">Cancel</button>
...
const PurchaseRow = ({ purchase, expanded, onToggle, onEdit, onDelete, onRestore, isDeleted }: {
  ...
  expanded: boolean; 
  onToggle: () => void
  onEdit: (p: StockPurchase) => void; 
  onDelete: (p: StockPurchase) => void
    ...
    <div className={`group ${isDeleted ? 'opacity-70' : ''}`}>
      <div onClick={onToggle}
        ...
        <div className="hidden lg:flex lg:col-span-3 items-center gap-1.5 flex-wrap">
          {purchase.supplier?.name && <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200 max-w-[110px] truncate"><svg className="w-3 h-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>{purchase.supplier.name}</span>}
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-mono font-medium text-gray-900">#{purchase.id}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Quantity:</span>
                  <span className="font-medium text-gray-900">{purchase.quantity} units</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Unit Price:</span>
                  <span className="font-medium text-gray-900">{fmt$(purchase.price_per_unit)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Shipping Cost:</span>
                  <span className="font-medium text-gray-900">{fmt$(purchase.shipping_cost)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Total Cost:</span>
                  <span className="font-medium text-gray-900">{fmt$(purchase.total_price_bought)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Min Sell Price:</span>
                  <span className="font-medium text-gray-900">{fmt$(purchase.min_price)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Max Sell Price:</span>
                  <span className="font-medium text-gray-900">{fmt$(purchase.max_price)}</span>
                </div>
                <div className="flex justify-between text-sm">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-900">{purchase.purchased_on ? fmtDate(purchase.purchased_on) : '—'}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Arrived:</span>
                  <span className="font-medium text-gray-900">{purchase.arrived_on ? fmtDate(purchase.arrived_on) : '—'}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Manufactured:</span>
                  <span className="font-medium text-gray-900">{purchase.manufacture_date ? fmtDate(purchase.manufacture_date) : '—'}</span>
                </div>
                <div className="flex justify-between text-sm">
              ...
              {purchase.supplier ? (
                <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                  ...
                  {purchase.supplier.contact && (
                    <p className="text-xs text-gray-500 mt-1">Contact: {purchase.supplier.contact}</p>
                  )}
                </div>
              ) : (
                ...
                <div className="relative" ref={dropdownRef}>
                  ...
                  {showDropdown && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10">
                      {isDeleted ? (
                        <button
                          onClick={(e) => { e.stopPropagation(); setShowDropdown(false); onRestore(purchase.id); }}
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); setShowDropdown(false); onDelete(purchase); }}
...
const StockPurchases = () => {
  ...
  const loadPurchases = useCallback(async (page = 1) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        // Normalize the API response to our StockPurchase type
        const normalizedPurchases = (res.data.items as ApiStockPurchase[]).map(item => normalizePurchase(item))
  ...
  const loadCounts = useCallback(async () => {
    try {
      ...
      if (activeRes.success && activeRes.data?.pagination) {
        setActiveCount(activeRes.data.pagination.total ?? 0)
      }
      if (deletedRes.success && deletedRes.data?.pagination) {
        setDeletedCount(deletedRes.data.pagination.total ?? 0)
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'active') {
      ...
      setDeletedCount(pagination.total)
    }
  ...
  const handleDelete = async (purchase: StockPurchase) => {
    try {
      ...
      addToast(err instanceof Error ? err.message : 'Failed to delete purchase', 'error')
    }
  }
  ...
  const handleRestore = async (id: number) => {
    try {
      ...
      addToast(err instanceof Error ? err.message : 'Failed to restore purchase', 'error')
    }
  }
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                        ...
                        {keys.map(key => (
                          <button
                            ...
                            className={`w-full px-4 py-2 text-left text-sm flex items-center justify-between transition-colors ${
                              sortKey === key ? 'bg-blue-50 text-blue-700 font-semibold' : 'hover:bg-gray-50 text-gray-700'
                            }`}
                          >
                            {sortLabels[key]}
                            {sortKey === key && (
              ...
              {activeTab === 'active' && (
                <button
                  onClick={() => { setEditPurchase(null); setShowModal(true); }}
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="hidden lg:grid grid-cols-12 gap-4 px-6 py-3.5 bg-gradient-to-r from-gray-50 to-gray-100/50 border-b border-gray-200">
            <div className="col-span-1"><span className="text-xs font-semibold text-gray-500 uppercase tracking-wider"></span></div>
            <div className="col-span-3"><span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">SKU / Product</span></div>
            <div className="col-span-1"><span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Qty</span></div>
            <div className="col-span-2"><span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total Cost</span></div>
            <div className="col-span-3"><span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Tags</span></div>
            <div className="col-span-2"><span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Purchased</span></div>
          </div>
          ...
          {loading ? (
            ...
            <div className="py-16 sm:py-24 px-4 text-center">
              ...
              <p className="text-sm text-gray-500 mb-6 max-w-sm mx-auto">
                {activeTab === 'deleted'
                  ? 'Deleted stock purchases will appear here.'
                  : 'Start tracking your inventory by recording your first stock purchase.'}
              </p>
              {activeTab === 'active' && (
                <button
