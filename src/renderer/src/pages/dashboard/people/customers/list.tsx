...
type SortKey = 'name_asc' | 'name_desc' | 'date_asc' | 'date_desc' | 'spent_asc' | 'spent_desc'
...
const sortToApiParams = (key: SortKey): { sort_by: string; sort_order: 'asc' | 'desc' } => {
  switch (key) {
    ...
    case 'spent_asc':  return { sort_by: 'total_spent', sort_order: 'asc' }
    case 'spent_desc': return { sort_by: 'total_spent', sort_order: 'desc' }
  }
}
...
const getCustomerGradient = (name: string) => {
  const idx = (name.charCodeAt(0) + (name.charCodeAt(1) || 0)) % tealGradients.length
  return tealGradients[idx]
}

const CustomerAvatar = ({ name, size = 'md', isDeleted }: { name: string; size?: 'sm' | 'md' | 'lg'; isDeleted?: boolean }) => {
  const sizeMap = { sm: 'w-8 h-8 text-xs', md: 'w-10 h-10 text-sm', lg: 'w-12 h-12 text-base' }
  const gradient = isDeleted ? 'from-gray-300 to-gray-400' : getCustomerGradient(name)
    ...
    {[...Array(5)].map((_, i) => (
      <div key={i} className="grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-4 px-4 lg:px-6 py-4 animate-pulse">
        ...
        <div className="hidden lg:block lg:col-span-2">
          <div className="h-6 bg-teal-100 rounded-full w-24" />
        </div>
        <div className="hidden lg:block lg:col-span-2">
...
const CustomerModal = ({ open, mode, initialData, customerId, onClose, onSuccess }: CustomerModalProps) => {
  ...
  const set = (patch: Partial<CustomerFormData>) => setForm(f => ({ ...f, ...patch }))
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'create') {
        const res = await window.api.customers.createCustomer({
        ...
        onSuccess(`"${form.customer_name}" has been added`)
      } else {
        const res = await window.api.customers.updateCustomer({
          id: customerId!,
          name: form.customer_name.trim(),
          phone: form.phone.trim() || undefined,
          email: form.email.trim() || undefined,
        ...
        if (!res.success) throw new Error(res.message)
        onSuccess(`"${form.customer_name}" has been updated`)
      }
      onClose()
    } catch (err) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-teal-50 to-white">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold text-gray-900">{mode === 'create' ? 'New Customer' : 'Edit Customer'}</h2>
          </div>
        ...
        <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <input
              ...
              onChange={e => { set({ customer_name: e.target.value }); clearErr('customer_name') }}
              placeholder="e.g. John Doe"
              className={`w-full px-4 py-3 rounded-lg border text-sm focus:outline-none focus:ring-2 transition-all ${errors.customer_name ? 'border-red-300 focus:ring-red-400 bg-red-50' : 'border-gray-300 focus:ring-teal-500 bg-white'}`}
            />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white transition-all"
              />
            </div>
            <div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-4 py-3 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white resize-none transition-all"
            />
          </div>
          ...
          <div className="flex items-center justify-between py-3 px-4 bg-gray-50 rounded-lg border border-gray-200">
            <div>
              ...
              <p className="text-xs text-gray-500">Visible and accessible in the system</p>
            </div>
            <button
              type="button"
              onClick={() => set({ is_active: !form.is_active })}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 ${form.is_active ? 'bg-teal-600' : 'bg-gray-300'}`}
            >
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50/50">
          ...
          <button
            ...
            disabled={submitting}
            className="px-5 py-2 bg-gradient-to-r from-teal-600 to-teal-500 hover:from-teal-700 hover:to-teal-600 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-lg transition-all shadow-md hover:shadow-lg flex items-center gap-2"
          >
            {submitting ? (
              <>
...
const CustomerActionModal = ({ mode, customer, onConfirm, onCancel }: CustomerActionModalProps) => {
  ...
  return (
    ...
    >
      <div
        ...
        onClick={e => e.stopPropagation()}
      >
        <div className={`px-6 pt-6 pb-4 ${isDelete ? 'bg-gradient-to-br from-red-50 to-red-100/40' : 'bg-gradient-to-br from-teal-50 to-teal-100/40'}`}>
          <div className="flex items-start gap-4">
            <CustomerAvatar name={customer.customer_name} size="lg" isDeleted={!isDelete} />
            <div className="flex-1 min-w-0">
              <h3 className="text-lg font-bold text-gray-900 truncate">{customer.customer_name}</h3>
              <p className="text-xs text-gray-500 mt-1">{isDelete ? 'Will be moved to deleted records' : 'Will be restored to active records'}</p>
            </div>
          </div>
        </div>
        <div className="px-6 py-5">
          <div className="flex items-start gap-3 mb-4">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${isDelete ? 'bg-red-100' : 'bg-teal-100'}`}>
              {isDelete ? (
                ...
                <svg className="w-5 h-5 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
              )}
            </div>
            <div>
              <h4 className="text-sm font-semibold text-gray-900 mb-0.5">{isDelete ? 'Delete Customer?' : 'Restore Customer?'}</h4>
              <p className="text-sm text-gray-600 leading-relaxed">
                {isDelete
                  ? 'All purchase history and records will be preserved. You can restore this customer at any time.'
                  : 'This customer will be restored and accessible in the system again.'}
              </p>
            </div>
          </div>
          <div className={`rounded-lg p-3 mb-5 border ${isDelete ? 'bg-amber-50 border-amber-200' : 'bg-teal-50 border-teal-200'}`}>
            <div className="flex gap-2">
              <svg className={`w-4 h-4 flex-shrink-0 mt-0.5 ${isDelete ? 'text-amber-600' : 'text-teal-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <p className={`text-xs leading-relaxed ${isDelete ? 'text-amber-800' : 'text-teal-800'}`}>
                {isDelete ? 'Purchase history, outstanding debt, and all records are kept for reporting purposes.' : 'The customer will be immediately active and available for new sales.'}
              </p>
            </div>
          </div>
...
const CustomerCard = ({ customer, expanded, onToggle, onDelete, onRestore, onEdit }: CustomerCardProps) => {
  ...
  return (
    <>
      <div className={`group ${customer.is_deleted ? 'opacity-70' : ''}`}>
        {/* ── Main Row ── */}
        <div
          onClick={onToggle}
          className={`
            ...
            ${expanded ? 'bg-teal-50/40' : ''} ${customer.is_deleted ? 'bg-red-50/20' : ''}
          `}
        >
          {/* Col 1: Avatar */}
          <div className="lg:col-span-1 flex items-center gap-3 lg:gap-0">
            <CustomerAvatar name={customer.customer_name} isDeleted={customer.is_deleted} />
            {/* Mobile: name inline */}
            <div className="lg:hidden flex-1 min-w-0">
              <div className="font-semibold text-gray-900 truncate">{customer.customer_name}</div>
              <div className="text-xs text-gray-500 mt-0.5">{customer.phone || customer.email || 'No contact'}</div>
            </div>
          </div>
          ...
          <div className="hidden lg:flex lg:col-span-3 flex-col justify-center min-w-0">
            <div className="font-semibold text-gray-900 truncate">{customer.customer_name}</div>
            <div className="text-xs text-gray-500 mt-0.5">ID: #{String(customer.id).padStart(4, '0')}</div>
          </div>
          ...
          <div className="hidden lg:flex lg:col-span-3 flex-col justify-center min-w-0">
            ...
            {!customer.phone && !customer.email && <span className="text-xs text-gray-400 italic">No contact info</span>}
          </div>

          {/* Col 4: Total spent */}
          <div className="hidden lg:flex lg:col-span-2 items-center">
            <div className="px-3 py-1.5 bg-teal-50 rounded-lg border border-teal-100">
              <div className="text-xs font-semibold text-teal-700">{formatCurrency(customer.purchase_stats?.total_spent)}</div>
              <div className="text-xs text-teal-500">{customer.purchase_stats?.total_purchases ?? 0} purchases</div>
            </div>
          </div>
          ...
          <div className="hidden lg:flex lg:col-span-2 items-center">
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${
              customer.is_deleted ? 'bg-red-50 text-red-700 border-red-200'
              : customer.is_active ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
              : 'bg-gray-100 text-gray-600 border-gray-200'
            }`}>
              <span className={`w-1.5 h-1.5 rounded-full ${customer.is_deleted ? 'bg-red-400' : customer.is_active ? 'bg-emerald-400' : 'bg-gray-400'}`} />
          ...
          <div className="hidden lg:flex lg:col-span-1 justify-end">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${expanded ? 'bg-teal-100 text-teal-600' : 'bg-gray-100 text-gray-500 group-hover:bg-gray-200'}`}>
        ...
        {/* ── Expanded Panel ── */}
        {expanded && (
          <div className="bg-gradient-to-br from-teal-50/30 to-gray-50/60 border-b border-gray-200">
            <div className="px-4 lg:px-8 py-5 grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1.5">
                  {[
                    { label: 'Total Purchases', value: `${customer.purchase_stats?.total_purchases ?? 0} orders` },
                    { label: 'Total Spent', value: formatCurrency(customer.purchase_stats?.total_spent) },
                    { label: 'Outstanding Debt', value: formatCurrency(customer.purchase_stats?.outstanding_debt), highlight: (customer.purchase_stats?.outstanding_debt ?? 0) > 0 },
                    { label: 'Last Purchase', value: formatDate(customer.purchase_stats?.last_purchase) },
                    { label: 'Customer Since', value: formatDate(customer.created_on) },
                  ].map(row => (
                    <div key={row.label} className={`p-2.5 rounded-lg transition-colors ${row.highlight ? 'bg-red-50/80 hover:bg-red-50' : 'bg-white/70 hover:bg-white'}`}>
                      <div className="text-xs text-gray-400 mb-0.5">{row.label}</div>
                      <div className={`text-sm font-medium ${row.highlight ? 'text-red-700' : 'text-gray-900'}`}>{row.value}</div>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <div className="relative" ref={dropdownRef}>
                    ...
                    {showDropdown && (
                      <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10">
                        {customer.is_deleted ? (
                          <button onClick={e => { e.stopPropagation(); setShowDropdown(false); setShowRestoreModal(true) }} className="w-full px-4 py-2.5 text-left text-sm flex items-center gap-3 hover:bg-teal-50 text-teal-600 transition-colors">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                            <span className="font-medium">Restore Customer</span>
                          </button>
                        ) : (
      ...
      {showDeleteModal && (
        <CustomerActionModal
          ...
          onConfirm={() => { onDelete?.(customer.id); setShowDeleteModal(false) }}
      ...
      {showRestoreModal && (
        <CustomerActionModal
          mode="restore"
          customer={{ id: customer.id, customer_name: customer.customer_name }}
          onConfirm={() => { onRestore?.(customer.id); setShowRestoreModal(false) }}
...
const Customers = () => {
  ...
  const loadCustomers = useCallback(async (page = 1) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        // Map the API response to match your Customer type
        const items = (res.data.customers || []).map((c: any) => ({
          id: c.id,
          ...
          purchase_stats: c.sales_stats ? {
            total_purchases: c.sales_stats.total_sales,
            total_spent: c.sales_stats.total_spent,
            outstanding_debt: c.sales_stats.outstanding_debt,
            last_purchase: c.sales_stats.last_purchase,
          } : undefined
        }))
        
        // Filter based on active tab
        const filteredItems = items.filter((c: Customer) =>
          activeTab === 'deleted' ? c.is_deleted : !c.is_deleted
        )
        ...
        if (pag) {
          setPagination({
            ...
            returned:    pag.returned    ?? filteredItems.length,
          })
          
          // Update counts based on pagination total
          if (activeTab === 'active') {
            setActiveCount(pag.total)
          } else {
            setDeletedCount(pag.total)
          }
        }
      } else {
        addToast(res.message || 'Failed to load customers', 'error')
      }
    } catch (err) {
      addToast('An error occurred while loading customers', 'error')
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  const handleDelete = async (customerId: number) => {
    const name = customers.find(c => c.id === customerId)?.customer_name ?? 'Customer'
    try {
      const res = await window.api.customers.softDeleteCustomer({ id: customerId })
      if (!res.success) throw new Error(res.message)
  ...
  const handleRestore = async (customerId: number) => {
    const name = customers.find(c => c.id === customerId)?.customer_name ?? 'Customer'
    try {
      const res = await window.api.customers.softDeleteCustomer({ id: customerId, restore: true })
      if (!res.success) throw new Error(res.message)
      ...
      addToast(err instanceof Error ? err.message : 'Failed to restore customer', 'error')
    }
  }
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-start gap-3">
              <div className="p-2.5 bg-gradient-to-br from-teal-500 to-teal-600 rounded-xl shadow-md flex-shrink-0">
            ...
            <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
              ...
              <div className="relative flex-1 sm:flex-initial">
                ...
                <input
                  ...
                  className="w-full sm:w-64 lg:w-72 pl-10 pr-10 py-2.5 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all bg-white"
                />
                {search && (
                ...
                {showSortMenu && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1.5 z-30">
                    {[
                      ...
                      { group: 'By Spending', keys: ['spent_desc', 'spent_asc'] as SortKey[] },
                    ].map(({ group, keys }, gi) => (
                      <div key={group}>
                        ...
                        {keys.map(k => (
                          <button key={k} onClick={() => { setSortKey(k); setCurrentPage(1); setShowSortMenu(false) }}
                            className={`w-full px-4 py-2 text-left text-sm flex items-center justify-between transition-colors ${sortKey === k ? 'bg-teal-50 text-teal-700 font-semibold' : 'hover:bg-gray-50 text-gray-700'}`}>
                            {sortLabels[k]}
              ...
              {activeTab === 'active' && (
                <button
                  ...
                  className="px-3 sm:px-5 py-2.5 bg-gradient-to-r from-teal-600 to-teal-500 hover:from-teal-700 hover:to-teal-600 text-white text-sm font-semibold rounded-lg transition-all shadow-md hover:shadow-lg flex items-center gap-2 whitespace-nowrap"
                >
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="flex border-b border-gray-200 px-4 sm:px-6 pt-3">
            {([
              { key: 'active' as TabKey, label: 'Active Customers', count: activeCount, color: 'teal' },
              ...
              <button
                ...
                className={`
                  ...
                  ${activeTab === tab.key
                    ? tab.color === 'teal' ? 'text-teal-600 border-teal-600' : 'text-red-600 border-red-500'
                    : 'text-gray-500 border-transparent hover:text-gray-700 hover:border-gray-300'
                  }
                `}
              >
                {tab.label}
                <span className={`inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-xs font-bold ${activeTab === tab.key ? tab.color === 'teal' ? 'bg-teal-100 text-teal-700' : 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500'}`}>
                  {tab.count}
                </span>
              </button>
            ))}
          ...
          {loading ? (
            ...
            <div className="py-16 sm:py-24 px-4 text-center">
              <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full mb-4 ${activeTab === 'deleted' ? 'bg-red-50' : 'bg-teal-50'}`}>
                {activeTab === 'deleted' ? (
              ...
              <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-2">
                {debouncedSearch ? 'No customers found' : activeTab === 'deleted' ? 'No deleted customers' : 'No customers yet'}
              </h3>
              <p className="text-sm text-gray-500 mb-6 max-w-sm mx-auto">
                {debouncedSearch ? `No results for "${debouncedSearch}".` : activeTab === 'deleted' ? 'Deleted customers will appear here.' : 'Start growing your customer base by adding your first customer.'}
              </p>
              {debouncedSearch
                ? <button onClick={() => setSearch('')} className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-teal-600 bg-teal-50 hover:bg-teal-100 rounded-lg transition-colors">Clear search</button>
                : activeTab === 'active' && (
                  <button onClick={() => setShowCreateModal(true)} className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-teal-600 to-teal-500 hover:from-teal-700 hover:to-teal-600 text-white text-sm font-semibold rounded-lg transition-all shadow-md hover:shadow-lg">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
            ...
            <div className="divide-y divide-gray-100">
              {customers.map(customer => (
                <CustomerCard
                  ...
                  onEdit={id => setEditCustomer(customers.find(c => c.id === id) ?? null)}
                />
              ))}
            </div>
          )}
        ...
        {!loading && pagination && pagination.total_pages > 1 && (
          <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-sm text-gray-500 order-2 sm:order-1">
              ...
              {' '}{activeTab === 'deleted' ? 'deleted' : 'active'} customers
            </p>
                ...
                {getPaginationRange().map((item, idx) =>
                  item === '…' ? (
                    ...
                    <button key={item} onClick={() => goToPage(item as number)}
                      className={`min-w-[36px] h-9 px-2 text-sm font-semibold rounded-lg transition-all ${item === pagination.page ? 'bg-teal-600 text-white shadow-md shadow-teal-200' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                      {item}
                    </button>
                  )
                )}
        ...
        {!loading && customers.length > 0 && pagination && pagination.total_pages <= 1 && (
          <div className="mt-4 sm:mt-6 text-center text-sm text-gray-500">
            Showing all {pagination.total} {activeTab === 'deleted' ? 'deleted' : 'active'} customers
          </div>
        )}
      </div>
    </div>
